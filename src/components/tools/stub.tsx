...
export default function ToolStub({ toolId, name }: StubProps) {
	return (
		<div className="h-fit rounded border border-terminal-border border-dashed bg-terminal-card/50 p-8">
			<h2 className="mb-2 font-bold font-mono text-matrix text-xl">{name}</h2>
			<p className="font-mono text-slate-400 text-sm">
				Status: Under Construction (Tool ID:{' '}
				<code className="rounded bg-muted px-1 py-0.5 text-slate-300">{toolId}</code>)
			</p>
